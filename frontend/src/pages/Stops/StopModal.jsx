...
export default function StopModal({ stop, onClose, onSave }) {
  ...
  const handleCoordinateChange = (e) => {
    ...
    if (value === "" || /^-?\d*\.?\d*$/.test(value)) {
      ...
      const lat =
        name === "vido" ? parseFloat(value) : parseFloat(formData.vido);
      const lng =
        name === "kinhdo" ? parseFloat(value) : parseFloat(formData.kinhdo);
      if (
        !isNaN(lat) &&
        !isNaN(lng) &&
        lat >= -90 &&
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
          {/* overflow-hidden + rounded-xl trên container chính để border-radius không bị cắt bởi header/footer */}

          {/* Header */}
          <div className="flex items-center justify-between p-6 bg-white">
            {/* Bỏ border-b để không cắt radius góc dưới */}
            <div className="flex items-center gap-3">
              <div>
                {/* Định dạng tiêu đề giống hệt ví dụ bạn muốn */}
                <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                  {stop ? (
                    <>
                      Chỉnh sửa điểm dừng – Mã:
          ...
          {/* Đường viền phân cách mỏng thay vì border-b dày để giữ bo góc */}
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  {/* Mã điểm dừng */}
                  {/* (Giữ nguyên chú thích này. Đã bỏ UI input như yêu cầu) */}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-3 py-2 border ${
                        errors.vido ? "border-red-500" : "border-gray-300"
                      } rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent`}
                      placeholder="10.8231"
                    />
                    {errors.vido && (
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleCoordinateChange}
                      className={`w-full px-3 py-2 border ${
                        errors.kinhdo ? "border-red-500" : "border-gray-300"
                      } rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent`}
